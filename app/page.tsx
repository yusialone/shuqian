...
export default function BookmarkPage() {
  ...
  useEffect(() => {
    setLoading(true)
    setError(null)
    fetch('https://shuqian.yusi1.dpdns.org/api/bookmarks')
      .then(res => res.json())
  ...
  const handleAddBookmark = async () => {
    if (newBookmark.title && newBookmark.url) {
      ...
      try {
        const res = await fetch('https://shuqian.yusi1.dpdns.org/api/bookmarks', {
          method: 'POST',
  ...
  const handleDeleteBookmark = async (id: string) => {
    ...
    setError(null)
    try {
      const res = await fetch(`https://shuqian.yusi1.dpdns.org/api/bookmarks/${id}`, { method: 'DELETE' })
  ...
  const handleEditSave = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`https://shuqian.yusi1.dpdns.org/api/bookmarks/${editingBookmark.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
